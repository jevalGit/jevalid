...
<head>
    <meta charset="UTF-8">
    <title>JevalID OAuth + OpenID Connect — Guía de implementación</title>
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <meta name="description"
          content="Guía completa para implementar el inicio de sesión con JevalID usando OAuth (Authorization Code) y OpenID Connect (OIDC), con ejemplos, errores comunes y buenas prácticas.">
...
<header class="py-4 mb-3 border-bottom border-secondary">
    <div class="container">
        <h1 class="mb-1">JevalID OAuth + OpenID Connect</h1>
...
<main class="container pb-5">
    ...
    <section id="oidc">
        <h2 class="section-title">2.1 OpenID Connect (OIDC): identidad estándar</h2>
        <p>
            Además del OAuth clásico, JevalID soporta <strong>OpenID Connect (OIDC)</strong> de forma <strong>opcional</strong>.
            OIDC es “OAuth + identidad estándar”. Si tu app pide <code>scope=openid</code>, entonces al canjear el <code>code</code>
            en <code>/oauth/token.php</code> recibirás también un <code>id_token</code> (JWT firmado).
        </p>
        <ul>
            <li><strong>Compatibilidad:</strong> apps antiguas que NO usan <code>scope=openid</code> siguen funcionando igual.</li>
            <li><strong>Ventaja:</strong> puedes validar identidad con un JWT estándar (sin depender siempre de <code>/api/userinfo.php</code>).</li>
        </ul>
        <p>
            Scopes OIDC soportados:
            <code>openid</code> (obligatorio), <code>email</code> (correo), <code>profile</code> (nombre).
        </p>
    </section>
...
&lt;/a&gt;</code></pre>
        ...
        <ul>
            ...
            <li><code>scope</code> — permisos solicitados. Para OIDC usa: <code>openid email profile</code>.</li>
            <li><code>nonce</code> — recomendado en OIDC para prevenir replay. Se devuelve dentro del <code>id_token</code>.</li>
        </ul>
...
Authorization: Bearer e4a9b2c9e91a4bc7...</code></pre>
  ...
  "user": {
    ...
    "rol": "admin"
        ...
        <h3>4.5 OIDC: validar el <code>id_token</code> (JWT)</h3>
        <p>
            Si tu app pidió <code>scope=openid</code>, el token endpoint devuelve <code>id_token</code>.
            Este token es un <strong>JWT</strong> firmado por JevalID (algoritmo <code>RS256</code>).
        </p>
        <p>Datos típicos dentro del <code>id_token</code>:</p>
        <ul>
            <li><code>iss</code>: <code>https://id.jeval.cl</code></li>
            <li><code>aud</code>: tu <code>client_id</code></li>
            <li><code>sub</code>: identificador estable del usuario (ej: <code>user:123</code>)</li>
            <li><code>exp</code>/<code>iat</code>: expiración y emisión</li>
            <li><code>nonce</code>: (si lo enviaste)</li>
            <li><code>email</code>/<code>name</code>: si pediste <code>email</code> y/o <code>profile</code></li>
        </ul>

        <h4>¿Cómo validarlo?</h4>
        <ol>
            <li>Decodifica el JWT y revisa el header: <code>alg=RS256</code> y <code>kid</code>.</li>
            <li>Obtén la clave pública desde <code>JWKS</code>:
                <code>https://id.jeval.cl/oauth/jwks.json</code></li>
            <li>Verifica firma RS256 con esa key pública.</li>
            <li>Valida claims:
                <ul>
                    <li><code>iss</code> debe ser <code>https://id.jeval.cl</code></li>
                    <li><code>aud</code> debe ser tu <code>client_id</code></li>
                    <li><code>exp</code> debe ser futuro</li>
                    <li><code>nonce</code> debe coincidir con el que guardaste en sesión (si lo usaste)</li>
                </ul>
            </li>
        </ol>

        <p class="text-warning">
            OIDC es opcional: si no quieres validar JWT, puedes seguir usando <code>/api/userinfo.php</code> con el <code>access_token</code>.
        </p>

        <h3>4.6 Descubrimiento OIDC (well-known)</h3>
        <p>
            JevalID publica el documento estándar:
            <code>https://id.jeval.cl/.well-known/openid-configuration</code>
...
exit;
    ...
    <section id="errors">
        ...
        <ul>
            ...
            <li><code>{"error":"client_revoked"}</code> (la app OAuth fue revocada)</li>
            <li><code>{"error":"oidc_key_missing"}</code> (faltan claves RS256 para emitir <code>id_token</code>)</li>
    ...
    <section id="best-practices">
        ...
        <ul>
            ...
            <li>Si usas OIDC, valida el <code>id_token</code> (firma RS256 + claims) usando <code>jwks.json</code>.</li>
